"""
수정된 Multi-head 실험: 각 헤드가 다른 영역을 보는 올바른 설계
- equality_head: 전반부 vs 후반부 비교 → 흐름 변화 감지
- ending_head: 마지막 64토큰 → 마무리 톤 감지
- threat_head: 전체 mean pool → 위협 신호 감지
비교 대상: 기존 Mean Pool (동일 조건)

실행: python -u src/run_proper_multihead.py > outputs/proper_multihead_log.txt 2>&1 &
"""
import pandas as pd
import numpy as np
import re
...
class MeanPoolClassifier(nn.Module):
    def __init__(self, model_name, num_classes=5, dropout_rate=0.3):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(h, 256), nn.GELU(), nn.Dropout(dropout_rate),
...
# ── 모델 B: 수정된 Multi-head (각 헤드가 다른 영역) ──
class ProperMultiHeadClassifier(nn.Module):
    def __init__(self, model_name, num_classes=5, dropout_rate=0.3):
        super().__init__()
        self.backbone = AutoModel.from_pretrained(model_name)
        h = self.backbone.config.hidden_size  # 768

        # equality: 전반/후반 비교 → diff(768) → 스칼라
        self.equality_head = nn.Sequential(
            nn.Linear(h, 128), nn.GELU(), nn.Dropout(dropout_rate), nn.Linear(128, 1))

        # ending: 마지막 64토큰 풀링(768) → 스칼라
        self.ending_head = nn.Sequential(
            nn.Linear(h, 128), nn.GELU(), nn.Dropout(dropout_rate), nn.Linear(128, 1))

        # threat: 전체 mean pool(768) → 스칼라
        self.threat_head = nn.Sequential(
            nn.Linear(h, 128), nn.GELU(), nn.Dropout(dropout_rate), nn.Linear(128, 1))

        self.layer_norm = nn.LayerNorm(h)

        # 768 (전체 pooled) + 3 (보조 스칼라) = 771 → 분류
        self.final_classifier = nn.Sequential(
            nn.Linear(h + 3, 256), nn.GELU(), nn.Dropout(dropout_rate),
            nn.Linear(256, num_classes))
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # --- equality: 전반부 vs 후반부 diff ---
        diffs = []
        for i in range(hidden.size(0)):
            L = int(seq_lens[i].item())
            mid = L // 2
            fm = torch.zeros(hidden.size(1), device=hidden.device)
            fm[1:mid] = 1.0
            sm = torch.zeros(hidden.size(1), device=hidden.device)
            sm[mid:L-1] = 1.0
        ...
        diff_vec = torch.stack(diffs)  # (B, 768)
        eq_score = self.equality_head(diff_vec)  # (B, 1)
        ...
        for i in range(hidden.size(0)):
            L = int(seq_lens[i].item())
            start = max(1, L - 64)
            em = torch.zeros(hidden.size(1), device=hidden.device)
            em[start:L-1] = 1.0
            end_pool = (hidden[i] * em.unsqueeze(-1)).sum(0) / em.sum().clamp(min=1e-9)
            endings.append(end_pool)
        end_vec = torch.stack(endings)  # (B, 768)
        end_score = self.ending_head(end_vec)  # (B, 1)

        # --- threat: 전체 mean pool ---
        threat_score = self.threat_head(pooled)  # (B, 1)
...
# ── 실험 실행 함수 ──
def run_experiment(exp_name, model_class):
    print(f'\n{"="*60}')
    print(f'[{exp_name}]')
    print(f'{"="*60}')
    ...
    torch.manual_seed(SEED); np.random.seed(SEED)
    model = model_class(MODEL_NAME, len(label2id), DROPOUT).to(device)
    ...
    save_path = f'outputs/best_model_{exp_name}.pt'
    ...
    for epoch in range(EPOCHS):
        ...
        marker = ' << BEST' if v_f1 > best_f1 else ''
        if v_f1 > best_f1:
    ...
    normal_f1 = classification_report(final_trues, final_preds, target_names=class_names, digits=4, output_dict=True)['일반 대화']['f1-score']
    ...
    sub.to_csv(f'outputs/submission_{exp_name}_str.csv', index=False)

    del model; import gc; gc.collect()
    return {'exp': exp_name, 'val_f1': best_f1, 'normal_f1': normal_f1, 'test_normal': test_normal, 'time': total_time}

# ── 메인: 두 모델 순차 실행 ──
print(f'구두점 유지 | Epoch {EPOCHS} | 동결 {FREEZE_LAYERS}층 | BS={BATCH_SIZE}')
print(f'Mean Pool vs Proper Multi-head 비교')

result = run_experiment('proper_multihead', ProperMultiHeadClassifier)

print(f'\n{"="*60}')
print(f'비교 (기존 Mean Pool 참고: Val F1 ~0.878, Test 일반 ~81건)')
print(f'{"="*60}')
print(f'  Proper Multi-head | Val F1: {result["val_f1"]:.4f} | 일반 F1: {result["normal_f1"]:.4f} | Test 일반: {result["test_normal"]}건 | {result["time"]:.1f}분')
