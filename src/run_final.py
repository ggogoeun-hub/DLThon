"""
Final Run: Multi-head + 동결 해제 + 최적 조합
실행: caffeinate -s & && python -u src/run_final.py > outputs/final_log.txt 2>&1 &
"""
import pandas as pd
import numpy as np
import re
...
LS = 0.1
DROPOUT = 0.3
FREEZE_LAYERS = 0  # 전체 해제

print(f'\n{"="*60}')
print(f'Final Run | Multi-head | Epoch {EPOCHS} | 동결 해제 | BS={BATCH_SIZE}')
print(f'Backbone LR: {BACKBONE_LR} | Head LR: {HEAD_LR} | LS: {LS}')
print(f'{"="*60}')
...
save_path = 'outputs/best_model_final.pt'
...
sub.to_csv('outputs/submission_final_str.csv', index=False)
sub_num = sub.copy()
sub_num['class'] = [label2id[c] for c in test_classes]
sub_num.to_csv('outputs/submission_final_num.csv', index=False)
print(f'\n  >> Submission 저장 완료')

print(f'\n{"="*60}')
print(f'Final Run 완료! | Val F1: {best_f1:.4f} | Test 일반: {test_normal}건 | {total_time:.1f}분')
print(f'{"="*60}')
